...
namespace vm68k
{
  namespace instr
  {
    ...
    uint32_type
    m68k_move(uint16_type op, context *c, void *)
    {
      Source ea1(op & 7, c->pc + 2);
      Destination ea2(op >> 9 & 7, c->pc + 2 + ea1.extension_size(Size()));
#ifdef L
      L("\tmove%s %s,%s\n", Size::suffix(), ea1.text(Size(), c).c_str(),
	ea2.text(Size(), c).c_str());
#endif

      typename Size::sint_type value = ea1.load_s(Size(), c);
      ea2.store(Size(), c, value);
      c->ccr.set_cc(value);

      ea1.finish(Size(), c);
      ea2.finish(Size(), c);
      return c->pc + 2 + ea1.extension_size(Size()) + ea2.extension_size(Size());
    }

    /* Handles a MOVEA instruction.  */
    template <class Size, class Source>
    uint32_type
    m68k_movea(uint16_type op, context *c, void *)
    {
      Source ea1(op & 7, c->pc + 2);
      int reg2 = op >> 9 & 7;
#ifdef L
	...
	reg2);
      ...
      long_word::sint_type value = ea1.load_s(Size(), c);
      (&c->a0)[reg2] = make_32(long_word(), (&c->a0)[reg2], value);

      ea1.finish(Size(), c);
      return c->pc + 2 + ea1.extension_size(Size());
    }
  }
